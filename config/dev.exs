...
config :amps_portal, AmpsPortal.Endpoint,
  ...
  http: [ip: {0, 0, 0, 0}, port: String.to_integer(System.get_env("AMPS_DEV_PORT", "4000")) + 1],
  check_origin: false,
  code_reloader: true,
  debug_errors: true,
...
config :amps_web, AmpsWeb.Endpoint,
  ...
  watchers: [
    # Start the esbuild watcher by calling Esbuild.install_and_run(:default, args)
    esbuild: {Esbuild, :install_and_run, [:amps_web, ~w(--sourcemap=inline --watch)]}
...
config :amps_web, AmpsWeb.Endpoint,
  live_reload: [
    patterns: [
      ~r"priv/static/.*(js|css|png|jpeg|jpg|gif|svg)$",
      ~r"priv/gettext/.*(po)$",
      ~r"lib/amps_web/(live|views)/.*(ex)$",
      ~r"lib/amps_web/templates/.*(eex)$"
    ]
  ]
