defmodule Amps.Handler do
  @callback initialize(state :: map()) :: map()
  @callback create_subscribers(state :: map()) :: boolean()

  defmacro __using__(usingopts) do
    quote do
      use Supervisor
      require Logger
      ...
      def init(args) do
        ...
        state = %{parms: parms, name: args[:name], env: env} |> initialize()

        state =
          if create_subscribers(args) do
            ...
            case Jetstream.API.Consumer.info(:gnat, stream, consumer) do
              {:error, %{"code" => 404}} ->
                AmpsUtil.create_config_consumer(parms, env, %{})
            ...
            state = Map.merge(state, %{stream: stream, consumer: consumer})
